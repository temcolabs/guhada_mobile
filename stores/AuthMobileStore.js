...
export default class AuthMobileStore {
  ...
  getCertKey = (location, childWindow, onAuthSuccess = () => {}) => {
    ...
    API.order
      ...
      .then((res) => {
        ...
        window.removeEventListener('message', this.onReceiveMessageFromPopup);
        window.addEventListener(
          'message',
          this.onReceiveMessageFromPopup,
          false
        );
      })
      .catch((e) => {
        childWindow.close();
        root.alert.showAlert('오류! 다시 시도해주세요.');
      })
      .finally(() => {
        this.access = false;
      });
  ...
  onReceiveMessageFromPopup = (event) => {
    // TODO: 인증완료 다음단계 진행
    let authData = event.data;
    let childWindow = this.childWindow;
    ...
    if (authData.sBirthDate) {
      ...
      if (location === 'findpassword') {
        this.checkIdentity(authData, afterResponse, afterException);
        ...
        this.onAuthSuccess(toJS(verifyParams));
