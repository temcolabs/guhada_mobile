...
describe('Error messages', function() {
  describe('first()', function() {
    it('should return an error message that states the email is required', function() {
      ...
      expect(validator.errors.first('email')).to.equal(
    ...
    it('should return error messages with attribute names and values for multi-part rules', function() {
      ...
      expect(validator.passes()).to.be.false;
      expect(validator.errors.first('age')).to.equal(
        'The age must be at least 18.'
      ); // min numeric
      expect(validator.errors.first('description')).to.equal(
        'The description must be at least 5 characters.'
      ); // min string
      expect(validator.errors.first('info')).to.equal(
        'The info field is required.'
      );
      expect(validator.errors.first('hours')).to.equal('The hours must be 5.'); // size numeric
      expect(validator.errors.first('pin')).to.equal(
        'The pin must be 4 characters.'
      ); // size string
      expect(validator.errors.first('range')).to.equal(
        'The range may not be greater than 10.'
      ); // max numeric
      expect(validator.errors.first('tweet')).to.equal(
    ...
    it('should fail without a matching confirmation field for the field under validation', function() {
      ...
      expect(validator.passes()).to.be.false;
      expect(validator.errors.first('password')).to.equal(
    ...
    it('should fail when the 2 attributes are the same', function() {
      ...
      expect(validator.passes()).to.be.false;
      expect(validator.errors.first('field2')).to.equal(
        'The field2 and field1 must be different.'
      );
    });

    it('should fail with a url only containing https://', function() {
      var link = 'https://';
      var validator = new Validator(
        {
          link: link,
      ...
      expect(validator.passes()).to.be.false;
      expect(validator.errors.first('link')).to.equal(
    ...
    it('should fail the custom telephone rule registration with a default error message', function() {
      ...
      expect(validator.passes()).to.be.false;
      expect(validator.errors.first('phone')).to.equal(
    ...
    it('should fail the custom telephone rule registration with a custom error message', function() {
      ...
      expect(validator.passes()).to.be.false;
      expect(validator.errors.first('cell')).to.equal(
  ...
  describe('should output correct error messages for numeric-like rules', function() {
    it('should give correct error message with numeric rule', function() {
      ...
      expect(validator.errors.first('val')).to.equal(
